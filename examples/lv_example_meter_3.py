"""
slightly modified lv_example_meter_3.py
"""
...
if size > 135:
    img_hand_min_file = 'img_hand_min.png'
    img_hand_hour_file = 'img_hand_hour.png'
else:
    img_hand_min_file = 'img_hand_min-sm.png'
    img_hand_hour_file = 'img_hand_hour-sm.png'

try:
    with open(img_hand_min_file, 'rb') as f:
        img_hand_min_data = f.read()
except:
    print("Could not find", img_hand_min_file)
    sys.exit()
...
# Create an image from the png file
try:
    with open(img_hand_hour_file, 'rb') as f:
        img_hand_hour_data = f.read()
except:
    print("Could not find", img_hand_hour_file)
    sys.exit()
...
meter.set_scale_major_ticks(scale_hour, 1, 2, 15, lv.color_black(), -21)         # Every tick is major
